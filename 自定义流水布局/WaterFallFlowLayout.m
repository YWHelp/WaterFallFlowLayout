...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // cell应该拼接的列数
    NSInteger destColumn = 0;//高度最小的那个cell的索引
    ...
    //当前cell列的高度就是当前cell的最大Y值(更新数组高度指)
    self.columnHeights[destColumn] = @(CGRectGetMaxY(attrs.frame));
    //self.columnHeights[destColumn] = @(@(CGRectGetHeight(attrs.frame)).floatValue  +  ((NSString *)[NSString stringWithFormat:@"%@",self.columnHeights[destColumn]]).floatValue);
